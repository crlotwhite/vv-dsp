...
vv_dsp_status vv_dsp_lpspec(const vv_dsp_real* a, size_t order, vv_dsp_real gain, size_t nfft, vv_dsp_real* mag_out) {
    ...
    for (size_t k=0;k<nfft;++k) {
        ...
        vv_dsp_real theta = (vv_dsp_real)(2.0*M_PI*(double)k/(double)nfft);
        vv_dsp_real re = 1.0, im = 0.0;
        for (size_t m=1; m<=order; ++m) {
            vv_dsp_real cr = cos((vv_dsp_real)m*theta);
            vv_dsp_real sr = sin((vv_dsp_real)m*theta);
            re += a[m]*(-cr);
            im += a[m]*(-sr);
        }
        vv_dsp_real den = (vv_dsp_real)sqrt((double)(re*re + im*im));
        mag_out[k] = (den > 0) ? (gain/den) : 0;
    }
    return VV_DSP_OK;
}
