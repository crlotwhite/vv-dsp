...
vv_dsp_status vv_dsp_window_hann(size_t N, vv_dsp_real* out) {
    ...
    const vv_dsp_real two_pi_over = (vv_dsp_real)(2.0 * M_PI) / denom;
    for (size_t n = 0; n < N; ++n) {
        vv_dsp_real c = (vv_dsp_real)cos(two_pi_over * (vv_dsp_real)n);
...
vv_dsp_status vv_dsp_window_blackman(size_t N, vv_dsp_real* out) {
    ...
    for (size_t n = 0; n < N; ++n) {
        ...
        vv_dsp_real c1 = (vv_dsp_real)cos(x);
        vv_dsp_real c2 = (vv_dsp_real)cos((vv_dsp_real)2.0 * x);
...
vv_dsp_status vv_dsp_window_blackman_harris(size_t N, vv_dsp_real* out) {
    ...
    for (size_t n = 0; n < N; ++n) {
        ...
        vv_dsp_real c1 = (vv_dsp_real)cos(x);
        vv_dsp_real c2 = (vv_dsp_real)cos((vv_dsp_real)2.0 * x);
        vv_dsp_real c3 = (vv_dsp_real)cos((vv_dsp_real)3.0 * x);
